...
const getters = {
    ...
    canSaveToLibrary(state: QueryState) {
        return !state.executing;
    },
    canSaveToNewPlaylist(state: QueryState) {
        return !state.executing && state.sink.new.name.length > 0;
    },
    canSaveToExistingPlaylist(state: QueryState) {
        return !state.executing && state.sink.existing.selectedId !== undefined;
    },
};
...
const mutations = {
    ...
    setAvailablePlaylists(store: QueryState, results: SpotifyApi.PlaylistObjectSimplified[]) {
        ...
        store.sink.existing.selectedId = results.length == 0 ? undefined : results[0].id;
    },
